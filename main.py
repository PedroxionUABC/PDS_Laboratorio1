...
presenta una falla o queda fuera de servicio, los administradores deben recibir una notificacion inmediata.
...
# Solucion
Implementamos el Patron Observador, donde el **Servidor Central** actua como el **Sujeto**, y los **Administradores** 
actuan como los **Observadores**. Cada administrador recibe una alerta cuando un servidor reporta una falla.

# Estructura
- Sujeto (SujetoConcreto - Servidor Central) Mantiene una lista de administradores y los notifica ante cambios.
- Observador (ObservadorConcreto - Administrador de Sistemas) Implementa la reaccion a la notificacion de fallos en servidores.
"""
...
if __name__ == "__main__":
    ...
    app.button = ctk.CTkButton(app, 
                               text="Reportar falla en el servidor a " + admin1.getNombre(), 
                               command=lambda:admin1.reportar_falla("No operativo"))
    app.button.grid(row=1, column=0, sticky="nsew")
    app.button = ctk.CTkButton(app,
                               fg_color='#e93100', 
                               text="Reportar falla en el servidor a " + admin2.getNombre(), 
